...
#define RAD2DEG (180 / TMath::Pi()) 
  ...
  UCorrelator::BinnedAnalyzer::Analyzer(const AnalysisConfig * conf, bool interactive_mode) 
: cfg(conf ? conf: &defaultConfig),
  bcorr(cfg->correlator_nphi,0,360,  cfg->correlator_ntheta, -cfg->correlator_theta_lowest, cfg->correlator_theta_highest, cfg->use_bin_center, cfg->scale_by_cos_theta, cfg->baseline_weight, cfg->correlation_gain_correction ) , 
...
void UCorrelator::Analyzer::analyze(const FilteredAnitaEvent * event, AnitaEventSummary * summary, const TruthAnitaEvent * truth) 
{
  ...
  for (int pol = cfg->start_pol; pol <= cfg->end_pol; pol++) 
  {
    ...
    corr.disallowAntennas(saturated[pol]);              // added sammy 2016_10_27: don't re-enable any antennas that are already disabled
...
void UCorrelator::Analyzer::fillWaveformInfo(const AnalysisWaveform * wf, const AnalysisWaveform * xpol_wf, const TGraph* pwr, AnitaEventSummary::WaveformInfo * info, AnitaPol::AnitaPol_t pol, double rms, double vpp)
{
  ...
  const TGraphAligned* hilbert = wf->hilbertEnvelope(); //added by sammy
  int peakBin;
